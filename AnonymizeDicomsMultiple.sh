...
BLUE='\033[0;34m'
NC='\033[0m'
echo "${BLUE}Running script to unzip and anonymize all DICOMs in the folder.${NC}"
...
if [ -z "$FileList" ]
then
  echo "${RED}Exiting script: No DICOM zip files found${NC}"
  exit 1
else
...
for i in $FileList; do
  ...
  echo "${BLUE}Generating SHA256...${NC}"
  ShaFull=$(sha256sum $i)
  echo "$ShaFull" >> BlindingKey.txt
  ShaSum=${ShaFull:0:64}
  echo "${GREEN}$ShaSum${NC}"
  ...
  unzip -qq $i -d "./$ShaSum"
  chmod -R 775 "./$ShaSum"
  echo "${GREEN}Zip file extracted.${NC}"
  ...
  rm ./$ShaSum/DICOM/*.bak
  NumDone=$((NumDone+1))
  echo "${GREEN}DICOM number $NumDone of $NumFiles ${NC}"
done
